...
import { Cancel01Icon, ClockIcon, FloppyDiskIcon, RocketIcon } from "@hugeicons/core-free-icons";
...
import { RequestHistoryDropdown } from "./request-history-dropdown";
...
export function RequestBar() {
  const {
    ...
    compactMode, setMethod, setUrl, clearActiveRequest, history,
  } = useAppStore();
          ...
          <div className={cn(
            ...
            {history.length > 0 && (
              <button
                ...
                className={cn(
                  "flex items-center justify-center rounded-lg transition-colors duration-150 shrink-0",
                  historyOpen
                    ? "text-ctp-lavender bg-ctp-lavender/10"
                    : "text-ctp-overlay0 hover:text-ctp-text hover:bg-ctp-surface0/30",
                  compactMode ? "w-6 h-6" : "w-7 h-7"
                )}
                title="Request History"
              >
                <HugeiconsIcon icon={ClockIcon} size={compactMode ? 12 : 14} strokeWidth={1.5} />
              </button>
            )}
